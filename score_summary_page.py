...
final_points_df["Total (with bonuses)"] = final_points_df.sum(axis=1, numeric_only=True)
...
# rider = "Jose Antonio Rueda"
# rider_dict = db.collection('scores').document(rider).get().to_dict()
# del rider_dict["championship_position"]
# del rider_dict["class_name"]
# scores = sum(rider_dict.values())
# st.write(f"{rider} has got: {scores}")
